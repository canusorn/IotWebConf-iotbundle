...
const char IOTWEBCONF_HTML_END[] PROGMEM          = "<script>var sysSpan = document.createElement('span');sysSpan.classList.add(\"tooltiptext\");sysSpan.classList.add(\"tooltip-right\");sysSpan.innerHTML = \"<b>ตั้งค่าเบื้องต้น</b><br><br>1.WIFI SSID-ใส่ชื่อไวไฟ<br><br>2.WIFI password-ใส่รหัสผ่านไวไฟ<br><br>3.อีเมล์ที่ได้สมัครที่เว็บ<br><br>4.รหัสผ่าน user\";var iwcsys = document.getElementById(\"iwcSys\");iwcsys.classList.add(\"tooltip\");iwcsys.appendChild(sysSpan);</script></div></body></html>";
